...
static std::string format_json(const TranscriptionResult &result, const std::string &source) {
  std::ostringstream ss;
  ss << "{\n"
...
static std::string format_rag(const TranscriptionResult &result, const std::string &source) {
  ...
  for (const auto &seg : result.segments) {
    ...
    bool at_boundary =
        !chunk.empty() && (chunk.back() == '.' || chunk.back() == '!' || chunk.back() == '?');
